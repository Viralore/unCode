//Program for Bubble Sort(in Descending Order) to sort the Array
...
void bubbleSortDescending(int arr[],int N)
{
    for(int i=0;i<N-1;i++)                  //loop to traverse the whole array
    {
        ...
        {
            if(arr[j]<arr[j+1])             //if the current element is less than the next element then swap the variable
            {
