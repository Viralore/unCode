//Program for Selection Sort(in Descending Order) to sort the Array
...
//In selection sort we assume 1st element to be maximum and store its index and then traverse the array
//if any other element is found greater than the assumed element then assign its index to temp var
//we traverse till last element and at last we swap the maximum element with the first element
//we make 2 parts in array sorted and unsorted
void selectionSortDescending(int arr[],int n)
{
    int max;                                //defining temporary var
    for(int i = 0;i<n-1;i++)                //traversing whole array
    {
        max = i;                            //assigning given element's index as maximum to the temp var
        for(int j=i+1;j<n;j++)              //traversing the array ahead of the taken element
        {
            if(arr[j]>arr[max])             //if other element found greater than the current element then assign its index to temp var
            {
                max = j;
            }
        }
        swap(arr[i],arr[max]);              //lastly swap the current element with the maximum element in sorted part of array
    }
}
