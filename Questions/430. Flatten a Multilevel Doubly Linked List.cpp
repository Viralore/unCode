...
{
public:
    Node* connect(Node *current,Node *temp)
    {
        Node *curr = current->child;
        while(curr->next!=NULL)
        {
            curr = curr->next;
        }
        curr->next = temp;
        /*if(curr->prev!=NULL) */temp->prev = curr;
        return current;
    }
...
int main()
{
    cout<<"Only Solution is provided"<<endl;
    cout<<"Cant code the main method :'(";
	return 0;
}
