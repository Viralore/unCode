//Program for deletion at start of linked list
//We will use structure to create a linked list for this

//Time Complexity of deletion at start
...
{
    if(head==nullptr) return nullptr;   //if list is empty return null pointer
    Node *temp = head->next;            //assigning second node to temp var
    delete head;                        //deleting the first node
    return temp;                        //returning temp var as head pointer
}
...
int main()
{
    ...
    cout<<endl<<"Printing list before deletion"<<endl;
    printLinkedList(head);                                      //printing the elements

    head = deletionAtStart(head);
    cout<<endl<<"Printing list after deletion"<<endl;
    printLinkedList(head);                                      //printing the elements
    return 0;
}
