//Program for deletion of last node in Singly Circular Linked List
...
Node* deleteLastNodeCircular(Node* head)
{
    if (head == nullptr)
    {
    ...
    // Traverse the circular linked list to reach the last node and find the node before it
    do
    {
        prev = current;
        current = current->next;
    ...
    prev->next = head;  // Update the next of the second-to-last node to make it circular
...
int main()
{
    ...
    head = deleteLastNodeCircular(head);
